...
async function migrateApplicableControlsAfterTreatmentToArray() {
  let client;
  ...
  try {
    ...
    // Find all risks that need migration (either string or missing applicableControlsAfterTreatment)
    const risksToUpdate = await risksCollection.find({
      $or: [
        { applicableControlsAfterTreatment: { $exists: true, $type: 'string' } },
        { applicableControlsAfterTreatment: { $exists: false } }
      ]
    }).toArray();
    ...
    for (let i = 0; i < risksToUpdate.length; i++) {
      ...
      try {
        ...
        const result = await risksCollection.updateOne(
          ...
          { 
            $set: { 
              applicableControlsAfterTreatment: selectedControlIds,
              updatedAt: new Date()
            } 
          }
        );
        ...
        if (result.modifiedCount > 0) {
          updatedCount++;
          const oldValue = risk.applicableControlsAfterTreatment || 'Not specified';
    ...
    if (errorCount > 0) {
      ...
      console.log('📝 Each risk now has 4-8 randomly selected SOA control IDs in applicableControlsAfterTreatment.');
    }
...
// Run the migration
migrateApplicableControlsAfterTreatmentToArray()
  .then(() => {
