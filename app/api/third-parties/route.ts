...
export async function GET() {
  try {
    ...
    // Check if third parties collection exists and has data
    ...
    if (existingData.length === 0) {
      ...
      console.log('Inserted fake third parties data')
    }
    
    // Retrieve all third parties
    const thirdParties = await collection.find({}).toArray()
    ...
    return NextResponse.json(
      { 
        ...
        data: fakeThirdParties // Fallback to fake data
      },
      { status: 500 }
    )
  }
