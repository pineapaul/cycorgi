...
export default function ThreatLibraryPage() {
  ...
  const [mitreMetadata, setMitreMetadata] = useState<MitreMetadata | null>(null)
  const [selectedTactics, setSelectedTactics] = useState<string[]>([])
  const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>([])
  const [filteredTechniques, setFilteredTechniques] = useState<MitreTechnique[]>([])
  
  // MITRE import state
  const [selectedMitreTechnique, setSelectedMitreTechnique] = useState<MitreTechnique | null>(null)
  ...
  const fetchThreats = useCallback(async (page = 1, newFilters = filters) => {
    try {
      ...
      if (result.success) {
        console.log('Threats API response:', result) // Debug log
        console.log('Threats data:', result.data) // Debug log
  ...
  const fetchMitreMetadata = useCallback(async () => {
    try {
      const response = await fetch('/api/mitre-attack/techniques?type=metadata')
      const result = await response.json()
  ...
  useEffect(() => {
    ...
    if (selectedPlatforms.length > 0) {
      filtered = filtered.filter(technique => 
        technique.platforms && technique.platforms.some(platform => 
    ...
    const selectedData = Array.from(selectedRows).map(index => threats[index])
     ...
     const csvContent = [
       ...
       ...selectedData.map(row => [
         ...
         row.mitreTechnique || '',
         row.source,
         row.informationAssets ? row.informationAssets.map(asset => asset.informationAsset).join('; ') : '',
         row.tags.join('; '),
         row.status,
    ...
    try {
      ...
      const response = await fetch('/api/threats', {
        ...
        body: JSON.stringify({
          ...
          status: 'Active',
          tags: [selectedMitreTechnique.tacticName.toLowerCase(), 'mitre-attack', selectedMitreTechnique.id.toLowerCase()],
      ...
      if (result.success) {
        ...
        showToast({
          type: 'success',
          title: 'Success',
          message: 'Successfully added ' + selectedMitreTechnique.name + ' as a threat!'
        })
      } else {
        showToast({
          type: 'error',
           ...
           return (
             <div className="flex flex-wrap gap-1">
               {value.map((asset: InformationAsset, index: number) => (
                 <span key={index} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded border border-green-200">
         ...
         return (
           <Tooltip content={
             <div className="space-y-1">
               <div className="font-medium text-white mb-2">Information Assets:</div>
               {value.map((asset: InformationAsset, index: number) => (
             ...
             <div className="flex flex-wrap gap-1">
               {value.slice(0, 2).map((asset: InformationAsset, index: number) => (
                 <span key={index} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded border border-green-200">
                   {escapeHtml(asset.informationAsset)}
                 </span>
               ))}
               ...
               {assetsLoading ? (
                 ...
                 <div className="max-h-48 overflow-y-auto border border-gray-300 rounded-md p-3">
                   {availableAssets.length === 0 ? (
                     <p className="text-sm text-gray-500 text-center py-4">No information assets available</p>
                   ) : (
                       ...
                       {availableAssets.map((asset) => (
                         <label key={asset.id} className="flex items-center space-x-3 cursor-pointer hover:bg-gray-50 p-2 rounded">
                           ...
                           <div className="flex-1">
                             <div className="font-medium text-sm text-gray-900">{asset.informationAsset}</div>
                             <div className="text-xs text-gray-500">{asset.category} • {asset.type} • {asset.criticality}</div>
                           </div>
                         </label>
                       ))}
                     </div>
          ...
          <div className="mb-6">
            ...
            {mitreMetadata && (
              <div className="space-y-4 mb-4">
                ...
                {/* Clear Filters */}
                {(selectedTactics.length > 0 || selectedPlatforms.length > 0 || searchTerm) && (
                  <div className="flex items-center space-x-2">
                    <button
                    ...
                    <span className="text-sm text-gray-500">
                      Showing {filteredTechniques.length} of {mitreTechniques.length} techniques
                    </span>
                  </div>
                )}
              </div>
                  ...
                  <div key={technique.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 line-clamp-2 mb-2">{technique.description}</p>
         ...
         <div className="p-6">
           <div className="mb-6">
             <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
               <h4 className="font-medium text-purple-900 mb-2">Selected MITRE Technique</h4>
               <div className="space-y-2">
                 ...
                 <h5 className="font-medium text-gray-900">{selectedMitreTechnique?.name}</h5>
                 <p className="text-sm text-gray-600">{selectedMitreTechnique?.description}</p>
               </div>
             </div>
